...
apps_installed_t* serialize_dict(PyObject *item) {
    ...
    if (dev_type != NULL) {
        ...
        ua->dev_type = PyUnicode_AsUTF8(dev_type);
    }
    ...
    if (dev_id != NULL) {
        ...
        ua->dev_id = PyUnicode_AsUTF8(dev_id);
    }
    ...
    if (lat != NULL) {
        ...
        *ua->lat = PyFloat_AsDouble(PyNumber_Float(lat));
    }
    ...
    if (lon != NULL) {
        ...
        *ua->lon = PyFloat_AsDouble(PyNumber_Float(lon));
    }
...
static PyObject* py_deviceapps_xwrite_pb(PyObject* self, PyObject* args) {
    ...
    printf("Write to: %s\n", path);
    return PyLong_FromLong(total_len);
}
